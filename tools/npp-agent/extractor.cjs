        ...
        if (index >= this.lastProcessedIndex) {
          try {
            ...
            newLogs.push({
              ...
              timestamp: data.timestamp || new Date().toISOString(),
              colorCode: data.colorCode || 'NV-808 (Xanh Ngọc Biển)',
  ...
  initSampleDbFile() {
    try {
      ...
      if (this.softwareType === 'CorobTINT') {
        ...
        const sampleJson = JSON.stringify({ id: `TINT-INIT-01`, colorCode: 'AP-102 (Vàng Kem)', totalVolumeLiters: 18, pigmentUsedMl: 95.0, status: 'HOÀN THÀNH', timestamp: new Date().toISOString() }) + '\n';
