...
BEGIN
  ...
  SET role = normalized_role, managed_region = normalized_region, updated_at = NOW()
  WHERE id = target_user_id
  RETURNING * INTO updated_profile;

  IF updated_profile.id IS NULL THEN RAISE EXCEPTION 'PROFILE_NOT_FOUND'; END IF;
  RETURN jsonb_build_object('id', updated_profile.id, 'role', updated_profile.role, 'managed_region', updated_profile.managed_region);
END;
$$;
